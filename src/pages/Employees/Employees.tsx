    ...
    <div className="p-6 md:p-10 mt-12 md:mt-0">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-ink-500 text-sm mt-1.5">{employees.length} çalışan</p>
        </div>
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 bg-plum-600 hover:bg-plum-700 text-white px-4 md:px-5 py-2.5 md:py-3 rounded-xl text-sm font-medium transition-all">
        ...
        {employees.map(emp => (
          <div key={emp.id} className="bg-white rounded-2xl p-6 shadow-sm border border-ink-100">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-12 h-12 bg-plum-600 rounded-full flex items-center justify-center text-white font-bold text-lg">
                {emp.name.charAt(0)}
              </div>
              <div>
                <h3 className="font-semibold text-ink-900">{emp.name}</h3>
                <p className="text-sm text-ink-500">{emp.phone || 'Telefon yok'}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 mb-5">
              <span className="text-xs bg-plum-100 text-plum-700 px-2.5 py-1.5 rounded-full font-medium">
                {emp.orders?.length || 0} Aktif Sipariş
              </span>
            </div>
            <div className="space-y-2.5">
              {emp.orders?.map((order: any) => (
                <div key={order.id} className="bg-ink-50 rounded-xl p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-plum-600 font-medium">#{order.id}</p>
              ...
              {(!emp.orders || emp.orders.length === 0) && (
                <p className="text-sm text-ink-400 text-center py-2">Aktif sipariş yok</p>
              )}
            </div>
          </div>
        ))}
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-7 w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-ink-900">Yeni Çalışan</h2>
              <button onClick={() => setShowForm(false)}><X size={20} className="text-ink-400" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-ink-600 mb-1">Ad Soyad</label>
              ...
              <div>
                <label className="block text-xs font-medium text-ink-600 mb-1">Telefon</label>
                <input value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })}
                  className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-plum-500"
                  placeholder="05XX XXX XX XX" />
              </div>
            </div>
            <button onClick={handleSave} className="w-full mt-6 bg-plum-600 hover:bg-plum-700 text-white font-medium py-2.5 rounded-xl transition-all">
              Kaydet
            </button>
          </div>
        </div>
