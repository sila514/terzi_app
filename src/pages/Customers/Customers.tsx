  ...
  return (
    <div className="p-8 md:p-14 mt-12 md:mt-0">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-semibold text-gray-900">Müşteri & Sipariş Takibi</h1>
          <p className="text-gray-500 text-sm mt-1.5">{customers.length} kayıtlı müşteri</p>
        </div>
        <button onClick={() => setShowCustomerForm(true)} className="flex items-center gap-2 bg-brand-700 hover:bg-brand-800 text-white px-4 md:px-5 py-2.5 md:py-3 rounded-xl text-sm font-medium transition-all">
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full pl-11 pr-4 py-3.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-600 bg-white"
        />
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-x-auto">
        <table className="w-full min-w-[500px]">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              <th className="text-left px-6 md:px-8 py-6 text-xs font-semibold text-gray-500 uppercase">Müşteri</th>
              <th className="text-left px-6 md:px-8 py-6 text-xs font-semibold text-gray-500 uppercase">Telefon</th>
              <th className="text-left px-6 md:px-8 py-6 text-xs font-semibold text-gray-500 uppercase">Sipariş</th>
              <th className="px-6 md:px-8 py-6"></th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {customers.map(c => (
              <tr key={c.id} onClick={() => loadCustomer(c.id)} className="hover:bg-brand-50 cursor-pointer transition-colors">
                <td className="px-6 md:px-8 py-6">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 md:w-10 md:h-10 bg-brand-700 rounded-full flex items-center justify-center text-white font-bold text-sm">
                      {c.name.charAt(0)}
                    </div>
                    <span className="font-medium text-gray-900 text-sm md:text-base">{c.name}</span>
                  </div>
                </td>
                <td className="px-6 md:px-8 py-6 text-sm text-gray-600">{c.phone || '-'}</td>
                <td className="px-6 md:px-8 py-6 text-sm text-gray-600">{c.orders?.length || 0}</td>
                <td className="px-6 md:px-8 py-6 text-right"><ChevronRight size={18} className="text-gray-400 ml-auto" /></td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 bg-black/30 flex justify-end z-50">
          <div className="w-full md:w-[600px] bg-white h-full overflow-y-auto shadow-2xl">
            <div className="p-6 md:p-8 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 md:w-11 md:h-11 bg-brand-700 rounded-full flex items-center justify-center text-white font-bold">
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => setShowOrderForm(true)} className="flex items-center gap-1 bg-brand-700 text-white px-3.5 py-2 rounded-lg text-sm font-medium">
            ...
            <div className="p-6 md:p-8">
              <h3 className="font-semibold text-gray-900 mb-4">Ölçü Kartı</h3>
              <div className="grid grid-cols-3 gap-3 md:gap-4 mb-8">
                {[['Göğüs', selected.chest], ['Bel', selected.waist], ['Kalça', selected.hip], ['Omuz', selected.shoulder], ['Boy', selected.height], ['Kol Boyu', selected.armLength]].map(([label, val]) => (
                  <div key={label as string} className="bg-gray-50 rounded-xl p-3 md:p-4">
                    ...
                    <p className="font-semibold text-gray-900 text-sm">{val ? `${val} cm` : '-'}</p>
                  </div>
                ))}
              </div>

              <h3 className="font-semibold text-gray-900 mb-4">Siparişler ({selected.orders?.length || 0})</h3>
              <div className="space-y-4">
                  ...
                  return (
                    <div key={order.id} className="border border-gray-100 rounded-xl p-5">
                      <div className="flex items-center justify-between mb-3">
                        <p className="font-medium text-gray-900 text-sm">{order.description || 'Sipariş'}</p>
                        ...
                        <div className="flex gap-2">
                          {kalan > 0 && (
                            <button onClick={() => { setSelectedOrder(order); setShowPaymentForm(true) }}
                              className="flex-1 flex items-center justify-center gap-1 bg-brand-700 hover:bg-brand-800 text-white py-2 rounded-lg text-xs md:text-sm font-medium transition-all">
                ...
                {(!selected.orders || selected.orders.length === 0) && (
                  <p className="text-gray-400 text-sm text-center py-4">Henüz sipariş yok</p>
                )}
              </div>
            </div>
          </div>
      ...
      {showPaymentForm && selectedOrder && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-2xl p-7 w-full max-w-md shadow-2xl">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Alınan Tutar (₺)</label>
            ...
            <button onClick={handlePayment} className="w-full mt-4 bg-brand-700 hover:bg-brand-800 text-white font-medium py-2.5 rounded-xl transition-all">
              Kaydet
            </button>
          </div>
        </div>
      ...
      {showOrderForm && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-2xl p-7 w-full max-w-xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-gray-900">Yeni Sipariş</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="md:col-span-2">
                <label className="block text-xs font-medium text-gray-600 mb-1">Model Notları</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Kapora (₺)</label>
                <input type="number" value={orderForm.depositPaid} onChange={e => setOrderForm({ ...orderForm, depositPaid: e.target.value })}
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-600" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Teslim Tarihi</label>
                <input type="date" value={orderForm.deliveryDate} onChange={e => setOrderForm({ ...orderForm, deliveryDate: e.target.value })}
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-600" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Çalışan</label>
            ...
            <button onClick={handleSaveOrder} className="w-full mt-6 bg-brand-700 hover:bg-brand-800 text-white font-medium py-2.5 rounded-xl transition-all">
              Kaydet
            </button>
          </div>
        </div>
      ...
      {showCustomerForm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-7 w-full max-w-xl shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <button onClick={handleSaveCustomer} className="w-full mt-6 bg-brand-700 hover:bg-brand-800 text-white font-medium py-2.5 rounded-xl transition-all">
              Kaydet
            </button>
          </div>
        </div>
