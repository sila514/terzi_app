  ...
  return (
    <div className="p-8 md:p-14 mt-12 md:mt-0">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-semibold text-gray-900">Muhasebe & Raporlama</h1>
          <p className="text-gray-500 text-sm mt-1.5">Finansal Özet</p>
        </div>
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 bg-brand-700 hover:bg-brand-800 text-white px-4 md:px-5 py-2.5 md:py-3 rounded-xl text-sm font-medium">
      ...
      <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-100 mb-9 md:mb-12">
        <h2 className="font-semibold text-gray-900 mb-5">Gelir & Gider Grafiği</h2>
        ...
        <div className="flex border-b border-gray-100 overflow-x-auto">
          {tabs.map(t => (
            <button key={t.key} onClick={() => setActiveTab(t.key)}
              className={`px-5 md:px-7 py-5 text-sm font-medium transition-all whitespace-nowrap ${activeTab === t.key ? 'border-b-2 border-brand-700 text-brand-700' : 'text-gray-500 hover:text-gray-700'}`}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        <div className="p-6 md:p-8">
          ...
          {activeTab === 'monthly' && (
            <div className="text-center py-4">
              <p className="text-3xl font-bold text-brand-700 mb-1">{summary?.monthlyRevenue?.toLocaleString('tr-TR') || 0} ₺</p>
              ...
              {!expenses.length ? (
                ...
                expenses.map((exp: any) => (
                  <div key={exp.id} className="flex items-center justify-between p-5 bg-gray-50 rounded-xl">
                    <div>
                      <p className="font-medium text-gray-900 text-sm">{exp.description || exp.category}</p>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-7 w-full max-w-lg shadow-2xl">
            ...
            <button onClick={handleSaveExpense} className="w-full mt-6 bg-brand-700 hover:bg-brand-800 text-white font-medium py-2.5 rounded-xl transition-all">
              Kaydet
            </button>
          </div>
        </div>
