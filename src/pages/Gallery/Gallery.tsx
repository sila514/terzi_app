    ...
    <div className="p-6 md:p-10 mt-12 md:mt-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div className="flex items-center gap-3">
          <select value={selectedOrder} onChange={e => setSelectedOrder(e.target.value)}
            className="flex-1 md:flex-none border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">
          ...
          <button onClick={() => fileRef.current?.click()} disabled={uploading}
            className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition-all disabled:opacity-50">
            <Upload size={18} />
      ...
      <div className="flex gap-2.5 mb-8 flex-wrap">
        <button onClick={() => setFilter('all')}
          className={`px-4 md:px-5 py-2 md:py-2.5 rounded-full text-sm font-medium transition-all ${filter === 'all' ? 'bg-purple-600 text-white' : 'bg-white text-gray-600 border border-gray-200'}`}>
          Tümü
        </button>
        {withPhotos.map(o => (
          <button key={o.id} onClick={() => setFilter(String(o.id))}
            className={`px-4 md:px-5 py-2 md:py-2.5 rounded-full text-sm font-medium transition-all ${filter === String(o.id) ? 'bg-purple-600 text-white' : 'bg-white text-gray-600 border border-gray-200'}`}>
            #{o.id}
          </button>
        ))}
      </div>
